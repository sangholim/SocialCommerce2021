...
import com.toy.project.service.ProductLabelQueryService;
...
public class ProductLabelResource {
    ...
    private final ProductLabelQueryService productLabelQueryService;

    public ProductLabelResource(
        ProductLabelService productLabelService,
        ProductLabelRepository productLabelRepository,
        ProductLabelQueryService productLabelQueryService
    ) {
        this.productLabelService = productLabelService;
        this.productLabelRepository = productLabelRepository;
        this.productLabelQueryService = productLabelQueryService;
    }
    ...
    public ResponseEntity<List<ProductLabelDTO>> getAllProductLabels(ProductLabelCriteria criteria, Pageable pageable) {
        log.debug("REST request to get ProductLabels by criteria: {}", criteria);
        Page<ProductLabelDTO> page = productLabelQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /product-labels/count} : count all the productLabels.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/product-labels/count")
    public ResponseEntity<Long> countProductLabels(ProductLabelCriteria criteria) {
        log.debug("REST request to count ProductLabels by criteria: {}", criteria);
        return ResponseEntity.ok().body(productLabelQueryService.countByCriteria(criteria));
    }
