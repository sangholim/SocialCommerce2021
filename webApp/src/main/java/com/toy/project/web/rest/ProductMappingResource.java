...
import com.toy.project.service.ProductMappingQueryService;
...
public class ProductMappingResource {
    ...
    private final ProductMappingQueryService productMappingQueryService;

    public ProductMappingResource(
        ProductMappingService productMappingService,
        ProductMappingRepository productMappingRepository,
        ProductMappingQueryService productMappingQueryService
    ) {
        this.productMappingService = productMappingService;
        this.productMappingRepository = productMappingRepository;
        this.productMappingQueryService = productMappingQueryService;
    }
    ...
    public ResponseEntity<List<ProductMappingDTO>> getAllProductMappings(ProductMappingCriteria criteria, Pageable pageable) {
        log.debug("REST request to get ProductMappings by criteria: {}", criteria);
        Page<ProductMappingDTO> page = productMappingQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /product-mappings/count} : count all the productMappings.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/product-mappings/count")
    public ResponseEntity<Long> countProductMappings(ProductMappingCriteria criteria) {
        log.debug("REST request to count ProductMappings by criteria: {}", criteria);
        return ResponseEntity.ok().body(productMappingQueryService.countByCriteria(criteria));
    }
